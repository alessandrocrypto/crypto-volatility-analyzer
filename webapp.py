...
async def analyze_command(update: Update, context: ContextTypes.DEFAULT_TYPE, days: int):
    ...
    for coin in crypto_results:
        response += (
            f"<b>{coin['symbol']}</b> — "
            f"{coin['average_volatility_points']:.1f} пунктов, "
            f"{coin['average_volatility_percent']:.2f}%\n"
        )

    if sp500_result:
        response += (
            f"<b>{sp500_result['symbol']}</b> — "
            f"{sp500_result['average_volatility_points']:.1f} пунктов, "
            f"{sp500_result['average_volatility_percent']:.2f}%\n"
        )
    else:
        response += "\n⚠️ S&P500 временно недоступен.\n"
...
async def daily_volatility_alert():
    ...
    if message_lines and TELEGRAM_CHAT_ID and bot:
...
def analyze(request: Request, days: int = Form(...)):
    ...
    try:
        ...
        print(f"⚠️ Ошибка generate_pdf_report: {e}")

    try:
        plot_data = get_plot_data()
    except Exception as e:
        print(f"⚠️ Ошибка get_plot_data: {e}")
        plot_data = {}
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "plot_data": plot_data,
            "crypto_tables": [coin.get("daily_data", []) for coin in crypto_results],
            "sp500_table": sp500_result.get("daily_data", []) if sp500_result else []
        }
    )
...
def download_report():
    ...
    if os.path.exists(filepath):
        return FileResponse(
            ...
            filename="volatility_report.pdf"
        )
...
async def telegram_webhook(request: Request):
    if not bot or application is None:
